...
df = pd.read_csv('fights_datas_rearranged.csv')
dateOfBith = df['DOB']
...
def test():
    ...
    for height1 in heightOfFigters1:
        if len(height1) > 0:
            height_list1.append(height1)
            # df.to_csv('clean_dataset.csv', index=False)
            # print(height)

    for i in range(len(height_list1)):
        height = height_list1[i].split("'")
        feet = int(height[0])
        inches = int(height[1])
        # feet1 = int(height1[0])
        # inches1 = int(height1[1])
        tot_inches = feet * 12 + inches
        meters = tot_inches * 0.0254
        df.replace(regex=[str(height_list1[i])], value=meters, inplace=True)
        print(len(height_list1), height_list1[i], i)
